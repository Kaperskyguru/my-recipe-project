      ...
      <div v-if="getSearchResults.length > 1">
        ...
        <div class="sm:grid sm:grid-cols-3 gap-5 w-4/5 sm:w-3/5 my-5 mx-auto">
          <div
            ...
            v-for="(item, i) in getSearchResults"
            :key="i"
          >
            ...
            <div class="p-5 shadow-lg">
              <div class="flex space-x-4">
                <button
                  @click="addItemToBookmark(item.recipe)"
                  class="
  ...
  methods: {
    ...
    async addItemToBookmark(item) {
      if (window.localStorage.getItem("userData")) {
        ...
        let bookmarkItem;
        if (
          this.bookmarks.findIndex((recipe) => recipe.label === item.label) ===
          -1
        ) {
          bookmarkItem = {
            label,
          ...
          await this.axios.post(
            `https://strapi-recipee-app.herokuapp.com/bookmarks`,
            {
              ...bookmarkItem,
            },
            {
          ...
          const res = await this.axios.get(
            `http://localhost:1337/users/${bookmarkItem.users_permissions_user}`,
            {
              headers: {
                Authorization: `Bearer ${window.localStorage.getItem("jwt")}`,
              },
